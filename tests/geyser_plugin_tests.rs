...
#[test]
fn test_account_notifications_disabled() {
    let plugin = GeyserPluginNats::new();
    assert!(!plugin.account_data_notifications_enabled());
}

#[test]
fn test_transaction_notifications_initially_disabled() {
    let plugin = GeyserPluginNats::new();
    // Should be disabled before loading configuration
    assert!(!plugin.transaction_notifications_enabled());
}
...
fn test_config_file_not_found() {
    ...
    if let Err(GeyserPluginError::ConfigFileReadError { .. }) = result {
        ...
        panic!("Expected ConfigFileReadError");
    }
}
...
#[test]
fn test_config_file_missing_required_fields() {
    let mut plugin = GeyserPluginNats::new();

    // Create a temporary file with valid JSON but missing required fields
    let temp_file = NamedTempFile::new().expect("Failed to create temp file");
    fs::write(&temp_file, r#"{"some_field": "some_value"}"#).expect("Failed to write to temp file");
...
fn test_valid_config_file() {
    ...
    let config = NatsPluginConfig {
        ...
        subject: "solana.transactions.test".to_string(),
        max_retries: 5,
        timeout_secs: 10,
        filter: TransactionFilterConfig::default(),
    };
    ...
    // Note: This might succeed if a NATS server happens to be running locally,
    // or fail if there's no server running. Both are valid outcomes for this test.
    let result = plugin.on_load(temp_file.path().to_str().unwrap(), false);

    // The config parsing should work regardless, so either:
    // 1. It succeeds (NATS server is running), or
    // 2. It fails with a NATS connection error (no server)
    // Both cases mean the config was parsed correctly
    if result.is_ok() {
        println!("NATS server was available, connection succeeded");
    } else {
        println!("NATS server was not available, connection failed as expected");
