...
fn test_serialize_transaction_v2_success() {
    ...
    assert_eq!(signatures[0].as_str().unwrap(), transaction.signatures()[0].to_string());
...
fn test_serialize_complex_transaction_v2() {
    ...
    let instructions = serialized["transaction"]["message"]["instructions"].as_array().unwrap();
...
fn test_serialize_multiple_transactions_consistency() {
    ...
    let transactions = vec![
        ...
        create_complex_test_transaction(),
    ];
    ...
    for (i, transaction) in transactions.iter().enumerate() {
        ...
        let result = TransactionSerializer::serialize_transaction_v2(&transaction_info, slot + i as u64);
