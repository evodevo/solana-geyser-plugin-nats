...
// Create a ConnectionManager for testing - it will try to connect but that's ok for testing
fn create_test_connection_manager() -> Arc<ConnectionManager> {
    ...
    // This keeps the worker thread alive long enough for our tests to run
    // The worker will keep retrying in the background while we test the processor logic
    match ConnectionManager::new("nats://127.0.0.1:9999", 100, 10) {
        ...
        Err(_) => {
            ...
            Arc::new(ConnectionManager::new("nats://localhost:9999", 100, 10)
...
mod processor_creation_tests {
    ...
    fn test_processor_new_with_default_config() {
        ...
        assert!(processor.transaction_selector().select_all_vote_transactions);
    }
    ...
    fn test_processor_new_with_specific_addresses() {
        ...
        assert!(!processor.transaction_selector().select_all_transactions);
        assert!(!processor.transaction_selector().select_all_vote_transactions);
        assert!(!processor.transaction_selector().mentioned_addresses.is_empty());
    }
...
#[cfg(test)]
mod transaction_selector_tests {
    use super::*;

    #[test]
    fn test_transaction_selector_all_transactions() {
        let connection_manager = create_test_connection_manager();
        let filter_config = TransactionFilterConfig {
            select_all_transactions: true,
            select_vote_transactions: false,
            mentioned_addresses: vec!["some_address".to_string()],
        };
        ...
        // When select_all_transactions is true, it should override other settings
        let selector = processor.transaction_selector();
        assert!(selector.select_all_transactions);
        assert!(selector.select_all_vote_transactions); // This is also set to true when select_all is true
    }

    #[test]
    fn test_transaction_selector_vote_only() {
        let connection_manager = create_test_connection_manager();
        let filter_config = TransactionFilterConfig {
            select_all_transactions: false,
            select_vote_transactions: true,
            mentioned_addresses: vec![], // Empty addresses
        };
        ...
        let selector = processor.transaction_selector();
        assert!(selector.select_all_vote_transactions);
        assert!(!selector.select_all_transactions);
    }

    #[test]
    fn test_transaction_selector_specific_addresses() {
        let connection_manager = create_test_connection_manager();
        let test_address = Pubkey::new_unique().to_string();
        let filter_config = TransactionFilterConfig {
            select_all_transactions: false,
        ...
        assert!(!selector.select_all_transactions);
        assert!(!selector.select_all_vote_transactions);
        assert!(!selector.mentioned_addresses.is_empty());
    }

    #[test]
    fn test_transaction_selector_default_fallback() {
        let connection_manager = create_test_connection_manager();
        let filter_config = TransactionFilterConfig {
            select_all_transactions: false,
            select_vote_transactions: false,
        ...
        // Should fall back to default behavior (select all)
        let selector = processor.transaction_selector();
        assert!(selector.select_all_transactions);
    }
}
...
mod error_handling_tests {
    ...
    #[test]
    fn test_processor_with_invalid_nats_url() {
        // Test with an invalid URL that should fail DNS resolution
    ...
    #[test]
    fn test_processor_with_invalid_port() {
        // Test with an invalid port number
        let result = ConnectionManager::new("nats://localhost:99999", 1, 1);
    ...
    fn test_processing_error_display() {
        use solana_geyser_plugin_nats::connection::ConnectionError;
        
        let connection_error = ConnectionError::SendFailed {
            msg: "Test error".to_string(),
        };
        let processing_error = ProcessingError::Connection(connection_error);
        assert!(processing_error.to_string().contains("Connection error"));
        
        let not_initialized_error = ProcessingError::NotInitialized {
            msg: "Test not initialized error".to_string(),
        };
        assert!(not_initialized_error.to_string().contains("Transaction processor not initialized"));
        
        let filtering_error = ProcessingError::FilteringFailed {
            msg: "Test filtering error".to_string(),
        };
        assert!(filtering_error.to_string().contains("Transaction filtering failed"));
    }
}
...
mod integration_tests {
    ...
    fn test_end_to_end_processing() {
        ...
        let result1 = processor.process_transaction(
            ReplicaTransactionInfoVersions::V0_0_2(&tx_v2), 
            12345
        );
        let result2 = processor.process_transaction(
            ReplicaTransactionInfoVersions::V0_0_1(&tx_v1), 
            12346
        );
        ...
        for thread_id in 0..num_threads {
            ...
            let handle = thread::spawn(move || {
                for tx_id in 0..transactions_per_thread {
                    ...
                    assert!(result.is_ok(), "Transaction processing failed in thread {}", thread_id);
                }
            });
            handles.push(handle);
        }
    ...
    async fn test_process_transactions_multiple_types() {
        ...
        let result_v1 = processor.process_transaction(
            ReplicaTransactionInfoVersions::V0_0_1(&v1_non_vote),
            12345,
        );
        let result_v2 = processor.process_transaction(
            ReplicaTransactionInfoVersions::V0_0_2(&v2_vote),
            12346,
        );
